...
export function Task4Workflow({ task, onBack, onContinue, status }: Task4WorkflowProps) {
  ...
  const handlePhotoSubmit = async () => {
    ...
    try {
      await createSubmissionMutation.mutateAsync({ 
        taskId: task.id, 
        file: selectedFile, 
        caption: "Photo of the dean of our Faculty"
      });
  ...
  const handleContinueToNextTask = async () => {
    try {
      // Don't change status here - it stays "in_review" until admin approves
      onContinue();
    } catch (error) {}
  };
  ...
  const renderMapPhase = () => (
    <Card className="glass-card border-0">
      ...
      <CardContent className="space-y-6">
        {/* Map Component - Task 4 coordinates */}
        <div className="h-80 w-full rounded-lg overflow-hidden border border-blue-300/30">
          <InteractiveMap 
            target={{ lat: 50.082381, lng: 14.413248 }} // Task 4 coordinates
            height="320px"
          />
        </div>
        ...
        {!justSubmitted ? (
          <>
            <div className="prose prose-invert max-w-none">
              <p>
                Every journey has a wise guide (and a slightly scary boss). Here is where you go for help, 
                advice, or explanations when you&apos;ve definitely messed something up. The First Student often 
                came here for help, advice, or (let&apos;s be honest) damage control.
              </p>
              <p>
                It is no secret that the First student was said to deliver their very first request here.
              </p>
              <p>
                <strong>
                  Your task is to honour his step. Find the photo of the dean of our Faculty and remember his face. 
                  Then download the photo and upload it here.
                </strong>
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex gap-2 items-center">
                <Badge variant="secondary" className="bg-yellow-600/20 text-yellow-300 border-yellow-400/30">
                ...
                {!selectedFile ? (
                  ...
                  >
                    <div className="text-center">
                      ...
                      <p className="text-sm text-blue-200">Click to select dean&apos;s photo</p>
                    </div>
                  </Button>
                ) : (
          ...
          <div className="text-center space-y-4">
            ...
            <p>Your photo has been sent for admin review.</p>
            <p className="text-sm text-white/70">Points will be awarded after approval.</p>
            <Button 
  ...
  const renderCompletedPhase = () => (
    <Card className="glass-card border-0">
      <CardHeader className="text-center">
        <CardTitle className="text-xl font-bold text-white">
          Great job, dear adventurers!
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6 text-white/90 text-center">
        <div className="prose prose-invert max-w-none">
          <p>
            Now your journey grows heavier. Can you move onward to the great halls of the Municipal Library?
          </p>
        </div>
        ...
        <div className="flex items-center justify-center gap-2 py-4">
          ...
          <span className="text-lg font-bold text-white">
            Task 4 Complete! Points pending admin review.
          </span>
        </div>
